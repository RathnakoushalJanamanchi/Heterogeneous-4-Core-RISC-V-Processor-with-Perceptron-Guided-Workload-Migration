    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (snoop_valid && (snoop_cmd == 2'b11)) begin
                if (valid[snoop_addr[4:2]] &&
                    (tags[snoop_addr[4:2]] == snoop_addr[31:12]))
                    mesi[snoop_addr[4:2]] <= `MESI_I;
            end
            if (mem_read) begin
...
module l2_cache (
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ack0<=1'b0; ack1<=1'b0;
            for(i=0;i<256;i=i+1)  mem[i] = 32'hDEADBEEF;
        end else if (retention_mode) begin
            ack0<=1'b0; ack1<=1'b0;
        end else begin
            ack0<=1'b0; ack1<=1'b0;
            if (req0) begin
                if (wr0) mem[addr0[9:2]] <= wdata0;
                else     rdata0 <= mem[addr0[9:2]];
                ack0 <= 1'b1;
            end
            if (req1) begin
                if (wr1) mem[addr1[9:2]] <= wdata1;
                else     rdata1 <= mem[addr1[9:2]];
                ack1 <= 1'b1;
            end
        end
    end
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (!active) begin
                ...
                EX_valid <= 1'b0; MEM_valid <= 1'b0; WB_valid <= 1'b0;
            end else begin
            branch_flush <= 1'b0;
            // PC update
